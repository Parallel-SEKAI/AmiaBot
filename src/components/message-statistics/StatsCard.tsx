      ...
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-4 gap-3">
          ...
          <StatBox
            icon={<Clock size={16} />}
            label="分析耗时"
            value={`${props.analysisTime}s`}
          />
          <StatBox
            icon={<Zap size={16} />}
            label="Token"
            value={props.tokenUsage}
          />
        </div>
        ...
        <section className="space-y-3">
          ...
          <div className="grid grid-cols-1 gap-3">
            {props.hot_topics.map((topic, idx) => (
              <div
                key={idx}
                className="p-3 bg-surface-container rounded-md border-l-4 border-primary"
              >
        ...
        <section className="space-y-3">
          ...
          <div className="grid grid-cols-2 gap-3">
            {props.group_members_titles.map((title, idx) => (
              <div
                key={idx}
                className="p-3 bg-secondary-container text-on-secondary-container rounded-md flex flex-col"
              >
          ...
          <section className="space-y-3">
            ...
            <div className="space-y-3">
              {props.group_bible.map((bible, idx) => (
                <div
                  key={idx}
                  className="relative p-4 bg-surface-container-high rounded-md border border-outline-variant"
                >
                  <div className="absolute top-2 right-4 text-4xl opacity-10 pointer-events-none text-primary">
                    "
                  </div>
