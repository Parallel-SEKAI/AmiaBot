      ...
      <div className="space-y-4">
        <div className="flex items-center gap-6 border-b-2 border-outline-variant pb-6 mb-4">
          ...
          <div className="flex-1">
            ...
            <div className="flex gap-2">
              <span
                className={`px-2 py-0.5 rounded text-[10px] font-bold uppercase ${props.role === 'owner' ? 'bg-error-container text-on-error-container' : props.role === 'admin' ? 'bg-primary-container text-on-primary-container' : 'bg-secondary-container text-on-secondary-container'}`}
              >
                {props.roleText}
              </span>
              {props.title && (
        ...
        <div className="grid grid-cols-2 gap-4">
          <section className="space-y-3">
            ...
            <div className="grid grid-cols-2 gap-2">
              ...
              <DetailItem
                label="Q龄"
                value={props.qAge ? `${props.qAge}年` : '未知'}
              />
              <DetailItem
                label="等级"
                value={props.qqLevel ? `Lv.${props.qqLevel}` : '未知'}
              />
            </div>
          </section>
        ...
        <div className="grid grid-cols-2 gap-4">
          <InfoBox
            icon={<Clock size={14} />}
            label="入群时间"
            value={props.joinTime || '未知'}
          />
          <InfoBox
            icon={<Clock size={14} />}
            label="最后发言"
            value={props.lastSentTime || '未知'}
          />
        </div>
      </div>
    </AppShell>
...
const DetailItem = ({
  ...
  <div className="flex flex-col p-2 bg-surface-container rounded border border-outline-variant/30">
    <div className="text-[9px] opacity-60 font-bold uppercase mb-0.5">
      {label}
    </div>
