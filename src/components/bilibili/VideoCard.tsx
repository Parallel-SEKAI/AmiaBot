    ...
    <AppShell>
      <div className="space-y-4">
        ...
        <div className="grid grid-cols-4 gap-2">
          <StatBox
            icon={<Play size={14} />}
            value={formatCount(props.play)}
            label="播放"
          />
          <StatBox
            icon={<ThumbsUp size={14} />}
            value={formatCount(props.thumbUp)}
            label="点赞"
          />
          <StatBox
            icon={<Database size={14} />}
            value={formatCount(props.coin)}
            label="硬币"
          />
          <StatBox
            icon={<Star size={14} />}
            value={formatCount(props.collect)}
            label="收藏"
          />
          <StatBox
            icon={<MessageSquare size={14} />}
            value={formatCount(props.danmaku)}
            label="弹幕"
          />
          <StatBox
            icon={<MessageSquare size={14} />}
            value={formatCount(props.reply)}
            label="评论"
          />
          <StatBox
            icon={<Share2 size={14} />}
            value={formatCount(props.share)}
            label="分享"
          />
          <StatBox
            icon={<Layers size={14} />}
        ...
        {props.intro && (
          <Container
            label="简介"
            variant="outline"
            className="text-xs text-on-surface-variant max-h-24 overflow-hidden relative bg-surface-container-low"
          >
            {props.intro}
          ...
          <div className="space-y-2">
            ...
            <div className="bg-surface-container rounded-md overflow-hidden border border-outline-variant">
              {props.pages.map((p, idx) => (
                <div
                  key={idx}
                  className="flex justify-between items-center p-2 text-xs border-b border-outline-variant last:border-b-0"
                >
        ...
        {props.seasons && props.seasons.length > 0 && (
          <div className="space-y-2">
            ...
            {props.seasons.map((s, idx) => (
              <div
                key={idx}
                className="bg-tertiary-container text-on-tertiary-container rounded-md p-2 space-y-1"
              >
                <div className="font-bold text-xs border-b border-on-tertiary-container/20 pb-1 mb-1">
                  {s.title}
                </div>
